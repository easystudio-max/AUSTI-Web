...
def get_google_sheet():
    try:
        ...
        st.error(f"❌ Google Sheets 연결 실패: {error_type}")
        st.info("""가능한 해결 방법:
1. 서비스 계정 이메일(austi-data-uploader@austi-research.iam.gserviceaccount.com)을 **Editor** 권한으로 공유했는지 확인
2. 스프레드시트의 '일반 액세스'를 '링크가 있는 모든 사용자 - 편집자'로 설정
3. Google Sheets API와 Google Drive API가 활성화되어 있는지 확인
4. 10~15분 정도 기다린 후 앱을 Redeploy 해보기""")
        return None
...
# ==================== Step 1: 20문항 검사 ====================
    ...
    questions = [
        ...
        "6. AI는 구체적인 업무를 빠르게 처리하는 데 최적이라고 믿는다.",
    ...
    if current < 20:
        ...
        button_text = "다음 문항" if current < 19 else "검사 완료하기"
...
# ==================== Step 3: 추가 설문 + Google Sheets 저장 ====================
    ...
    if st.button("모든 데이터 제출하기", type="primary"):
        ...
        if sheet is not None:
            ...
            st.error("Google Sheets 연결에 실패했습니다. 권한 설정을 다시 확인해주세요.")
