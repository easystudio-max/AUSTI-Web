...
def get_google_sheet():
    try:
        ...
        # 교수님 새 Spreadsheet ID
        spreadsheet = gc.open_by_key("1u6GIzlf0jnUT_1EHzdyfPvxrdNv2r-58mO2UFITAYbo")
        sheet = spreadsheet.sheet1
        st.success("✅ Google Sheets 연결 성공! 데이터 저장 준비 완료.")
        return sheet
    except Exception as e:
        st.error(f"❌ 연결 실패: {type(e).__name__}")
        st.info("🔧 해결 방법:\n"
                "1. 서비스 계정 이메일을 스프레드시트에 **Editor**로 공유했는지\n"
                "2. '일반 액세스'를 '링크가 있는 모든 사용자 - 편집자'로 설정\n"
                "3. Google Drive API와 Google Sheets API가 활성화되었는지\n"
                "4. 10~15분 기다린 후 Redeploy")
        return None
...
if st.session_state.step == 0:
    ...
    st.write("본 검사는 충북보건과학대학교 글로벌IT학과 정인훈 교수팀의 연구에 활용되며, 모든 데이터는 익명 처리되어 학술 목적으로만 사용됩니다.")
...
# Step 3: 추가 설문 + 저장
    ...
    if st.button("모든 데이터 제출하기", type="primary"):
        ...
        if sheet is not None:
            ...
            st.error("Google Sheets 연결 실패. 위 안내를 따라 권한을 다시 확인해주세요.")

# Step 4: 완료
elif st.session_state.step == 4:
    st.success("🎉 모든 검사가 완료되었습니다! 감사합니다.")
    st.balloons()
